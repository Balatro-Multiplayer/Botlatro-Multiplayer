...
module.exports = {
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const membersPerTeam =
      interaction.options.getInteger('members-per-team', false) ?? 1
    const numberOfTeams =
      interaction.options.getInteger('number-of-teams', false) ?? 2
    const eloSearchStart =
      interaction.options.getInteger('queue-elo-search-start', false) ?? 0
    const eloSearchIncrement =
      interaction.options.getInteger('queue-elo-search-increment', false) ?? 1
    const eloSearchSpeed =
      interaction.options.getInteger('queue-elo-search-speed', false) ?? 1
    const minimumElo = interaction.options.getInteger('minimum-elo', false) ?? 0
    const maximumElo =
      interaction.options.getInteger('maximum-elo', false) ?? 10000
    ...
    try {
      ...
      if (!nameIsAvailable) {
        await interaction.reply({
          content: 'A queue with that name already exists.',
      ...
      await interaction.reply({
        content: `Successfully created queue ${queueName}.`,
        flags: MessageFlags.Ephemeral,
      })
    } catch (err: any) {
      console.error(err)
      ...
      if (interaction.deferred || interaction.replied) {
        await interaction.editReply({
          content: `Failed to create queue in database. Reason: ${errorMsg}`,
        })
      } else {
        await interaction.reply({
          content: `Failed to create queue in database. Reason: ${errorMsg}`,
