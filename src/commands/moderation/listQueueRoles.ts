...
export default {
  async execute(interaction: ChatInputCommandInteraction) {
    try {
        ...
        const formattedQueueRoles = queueRoles.map(role => {
            const roleData = guild.roles.cache.get(role.role_id);
            if (roleData) {
                return `**${roleData.name}**: ${role.mmr_threshold}+ MMR`;
            } else {
                return `**N/A**: ${role.mmr_threshold}+ MMR`;
            }
        }).join('\n');


        if (queueRoles.length > 0) {
            await interaction.reply({ content: `**Queue Roles for ${queueName}**\n${formattedQueueRoles}` });
        } else {
            await interaction.reply({ content: `No queue roles have been added to this queue.` });
        }
    } catch (err: any) {
        console.error(err)
