...
export default {
  async execute(interaction: ChatInputCommandInteraction) {
    try {
        ...
        const reference = interaction.options.getChannel('reference channel', false) || discordChannel;
        ...
        const expiryDate = await calculateExpiryDate()

        strikeUtils.addStrike({
            id: 0, // This will be auto-generated by the database
            user_id: user.id,
            reason: reason,
            issued_by_id: interaction.user.id,
            issued_at: new Date(),
            expires_at: expiryDate || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000),
            amount: amount,
            reference: referenceChannel
        })

        const threadId = await getLoggingThread(1)
