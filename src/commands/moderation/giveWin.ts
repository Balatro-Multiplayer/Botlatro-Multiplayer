...
module.exports = {
	data: new SlashCommandBuilder()
        ...
        .addIntegerOption(option =>
            option.setName('match-id')
                .setDescription('The match ID to assign a win for (defaults to current channel)')
                .setRequired(true)
    ...
    async autocomplete(interaction: AutocompleteInteraction) {
        ...
        if (currentValue.name === 'user') {
            ...
            const input = currentValue.value
            const matches = await pool.query('SELECT id FROM matches WHERE open = true');
            if (!matches.rows || matches.rows.length === 0) {
                await interaction.respond([]);
                return;
            }
            const matchIds = matches.rows.map((match: any) => ({
                name: (interaction.channelId = match.id.toString()) ? `${match.id.toString()} (current channel)` : match.id.toString(),
                value: match.id.toString()
            }));
            const filtered = matchIds.filter(match =>
