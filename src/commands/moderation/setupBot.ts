...
module.exports = {
	...
	async execute(interaction: ChatInputCommandInteraction) {
		...
		try {
			const queueCategoryId = interaction.options.getChannel('queue-category', true)?.id;
		...
		try {
			...
			async function getOrCreateChannel(channelId: string | null, channelName: string, permissionOverwrites: any, databaseEntry: string) {
			...
			const settingsRes = await pool.query('SELECT * FROM settings WHERE singleton = true');
			...
			const everyonePermsSend = [{ id: guild.roles.everyone, deny: [PermissionFlagsBits.SendMessages] }];
			const everyonePermsView = [{ id: guild.roles.everyone, deny: [PermissionFlagsBits.ViewChannel] }];
			const helperPerms = { id: helperRole.id, allow: [PermissionFlagsBits.ViewChannel], deny: [PermissionFlagsBits.SendMessages] };
			...
			await getOrCreateChannel(settings.queue_logs_channel_id, 'queue-log', [everyonePermsView[0], helperPerms], 'queue_logs_channel_id')
