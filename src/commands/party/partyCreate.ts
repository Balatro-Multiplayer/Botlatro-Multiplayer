...
module.exports = {
	async execute(interaction: ChatInputCommandInteraction) {
        ...
        try {
            ...
            const partyName = interaction.options.getString('party-name') || `${interaction.user.username}'s Party`;
            const partyId = await partyUtils.getUserParty(userId);

            if (partyId) {
                await interaction.editReply({ content: `You are already in a party. Leave your current party before creating a new one.` });
                return;
            }
            ...
            if (interaction.deferred || interaction.replied) {
                ...
                await interaction.reply({ content: `Failed to create party.`, flags: MessageFlags.Ephemeral });
            }
        }   
    }
};