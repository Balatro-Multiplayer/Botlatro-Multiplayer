	...
	async execute(interaction: ChatInputCommandInteraction) {
		...
		try {
            ...
            if (!parties || parties.length === 0) {
                await interaction.editReply({ content: `There are currently no parties.` });
                return;
            }
            ...
            await interaction.editReply({ content: `All Parties: \n${partyList.join('\n')}` });
        } catch (err: any) {
            console.error(err);
            if (interaction.deferred || interaction.replied) {
                await interaction.editReply({ content: `Failed to list parties.` });
            } else {
                await interaction.reply({ content: `Failed to list parties.`, flags: MessageFlags.Ephemeral });
            }
        }
    }
};